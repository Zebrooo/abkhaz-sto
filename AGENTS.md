...
- **Вход — на сайте, выход — здесь.** Кука сессии общая для поддоменов
  (`aa-auth-token`, `.abkhaz-auto.ru`, см. `src/lib/auth-cookies.ts`); своих
  экранов ввода кода нет. Имя и домен куки обязаны совпадать с сайтом.
  Выход — свой (`src/app/auth-actions.ts`): гасит сессию в GoTrue
  (`scope: "local"`) и стирает куку вместе с чанками, обязательно с доменом.
  Обе половины нужны: без GoTrue мобильная оболочка вернёт те же токены
  мостом, без домена браузер сотрёт куку не ту.

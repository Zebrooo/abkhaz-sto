...
/**
 * Куда на самом деле ведёт «Осмотр»: мастеру — на осмотр его текущей записи,
 * админу и хозяину — на ту, что идёт сейчас на любом посту. Вкладки и меню
 * ссылаются на постоянный /osmotr (lib/nav.ts), а расчёт делает его
 * страница-редиректор — один раз при переходе, а не на каждый рендер layout.
 *

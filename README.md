...
| `/segodnya` | записи дня таймлайном по постам: линия «сейчас», свободные окна, фильтры статусов, перенос записи перетаскиванием |
| `/kalendar` | неделя столбиками загрузки и записи выбранного дня; перенос — списком свободных окон или перетаскиванием записи на другой день |
...
| `/mashiny/[ключ]` | карточка машины: владельцы (её продают — история остаётся), визиты, деньги, гараж клиента с сайта |
...
| `/menu` | «Ещё»: вход в услуги, расписание, уведомления, витрину на сайте и выход из приложения |
| `/zapis/[id]/osmotr`, `/zapis/[id]/otchet` | осмотр с фото дефектов (норму не отмечают) и отчёт по диагностике со сметой |
| `/otchety` | готовые отчёты за период (сегодня / 7 / 30 дней) по дням осмотра; мастеру — свои |
...
Потянуть экран вниз у самого верха — обновить его (`components/PullRefresh.tsx`,
правила жеста и их тесты — `src/lib/pull.ts`): записи приходят с сайта сами, а
в оболочке нет ни адресной строки, ни кнопки перезагрузки.
...
Выход, наоборот, свой: «Ещё» → «Выйти» (на вебе — в подвале разделов слева),
а на экранах «Организации нет» и «Вход для автосервиса» — кнопка в карточке.
`src/app/auth-actions.ts` гасит сессию в GoTrue и стирает куку с доменом:
одной стёртой куки мало, мобильная оболочка держит те же токены и вернула бы
их мостом.

...
describe("свободные куски поста на сетке", () => {
  ...
  it("делит интервал по записям; кусок в шаг сетки — уже окно", () => {
    const gaps = freeGaps([{ from: "09:00", to: "13:00" }], [
      { from: 9 * 60 + 30, to: 10 * 60 },      // до неё 30 минут — при шаге 30 это окно
      { from: 11 * 60, to: 12 * 60 },          // между 10:00 и 11:00 — час, окно
    ]);
    expect(gaps).toEqual([
      { fromMin: 9 * 60, toMin: 9 * 60 + 30 },
      { fromMin: 10 * 60, toMin: 11 * 60 },
      { fromMin: 12 * 60, toMin: 13 * 60 },
    ]);
  });

  it("кусок короче шага сетки не показывает: запись в него не встанет", () => {
    const gaps = freeGaps([{ from: "09:00", to: "13:00" }], [
      { from: 9 * 60, to: 9 * 60 + 45 },       // 09:45–10:00 — 15 минут при шаге 30, не окно
      { from: 10 * 60, to: 11 * 60 },
    ]);
    expect(gaps).toEqual([{ fromMin: 11 * 60, toMin: 13 * 60 }]);
  });

  it("окно начинается на узле сетки, как в freeSlots", () => {
    // Запись до 09:30 при шаге 20: следующий узел от 09:00 — 09:40.
    expect(freeGaps([{ from: "09:00", to: "13:00" }], [{ from: 9 * 60, to: 9 * 60 + 30 }], { stepMin: 20, bufferMin: 0 }))
      .toEqual([{ fromMin: 9 * 60 + 40, toMin: 13 * 60 }]);
  });

  it("буфер держит пост после записи и перед следующей, окно — со следующего узла сетки", () => {
    // 09:00–10:00 плюс буфер 10 — пост занят до 10:10, узел сетки — 10:30.
    expect(freeGaps([{ from: "09:00", to: "13:00" }], [{ from: 9 * 60, to: 10 * 60 }], { stepMin: 30, bufferMin: 10 }))
      .toEqual([{ fromMin: 10 * 60 + 30, toMin: 13 * 60 }]);
    // Перед записью 12:00 буфер тоже нужен (postTaken): окно кончается в 11:50.
    expect(freeGaps([{ from: "09:00", to: "13:00" }], [{ from: 9 * 60, to: 10 * 60 }, { from: 12 * 60, to: 13 * 60 }], { stepMin: 30, bufferMin: 10 }))
      .toEqual([{ fromMin: 10 * 60 + 30, toMin: 11 * 60 + 50 }]);
    // Буфер вылез за конец интервала — окна нет, а не отрицательное окно.
    expect(freeGaps([{ from: "09:00", to: "10:00" }], [{ from: 9 * 60, to: 9 * 60 + 55 }], { stepMin: 30, bufferMin: 10 }))
      .toEqual([]);
  });
  ...
  it("раньше «сейчас плюс lead» окон не предлагает, как форма записи", () => {
    // Сейчас 09:50, lead час — первый узел сетки после 10:50 это 11:00.
    expect(freeGaps(work, [], { stepMin: 30, bufferMin: 0 }, 10 * 60 + 50))
      .toEqual([{ fromMin: 11 * 60, toMin: 13 * 60 }, { fromMin: 14 * 60, toMin: 18 * 60 }]);
    // Прошедший день — окон нет вовсе.
    expect(freeGaps(work, [], { stepMin: 30, bufferMin: 0 }, Infinity)).toEqual([]);
  });
});
